...
def start_experiment(exp_name, app, kernel):
  if kernel == "clean":
    ...
    kernel_mode = "1"
  cmd = def_cmd + ' $REPO_ROOT/atc_measurements/' + app + '/record-mlperf.sh stub 2 0 ' + kernel_mode
  run(cmd)

@roles('clients')
def prefault_hostmem():
  with settings(hide('warnings'), warn_only=True,):
    cmd = def_cmd + '$REPO_ROOT/atc_measurements/prefault_hostmem/alloc_120GB.sh'
    run(cmd)
...
@run_bg('servers')
def start_vm(kernel, dataset):
  run('sudo numactl --membind=0 --cpunodebind=0 qemu-system-x86_64 -kernel ' + kernel_dir '/' + kernel + '/arch/x86/boot/bzImage -boot c -m 64GB -hda ' + image_dir + '/rootfs.img -append "root=/dev/sda rw transparent_hugepage=never" -device e1000,netdev=net0 -netdev user,id=net0,hostfwd=tcp::6666-:22 --enable-kvm -smp 20 -cpu host -nographic')
